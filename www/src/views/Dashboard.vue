...
export default {
      ...
      eventsTable: reactive({
        ...
        columns: [
          ...
          },
          {
            ...
            columnStyles: { "text-align": "center" },
            display: function (row) {
              const d = new Date(row[this.field] * 1000);
          ...
          },
          {
            ...
            display: function (row) {
              return (
                '<a href="' + row[this.field] + '">' + row[this.field] + "</a>"
              ); // FIXME: Sanitize this
            },
          },
          {
  ...
  mounted() {
    const sortmode = (a, b) => b.id - a.id;
    queryDB(
      "query{keywords{id,timestamp:createdAt,type,value,lastConsulted}events{id,timestamp:createdAt,template,type,source,data}}",
      (data) => {
  ...
  methods: {
    filterEvents() {
      ...
      // create the dorks
      const dorks = search.split(" ").filter((d) => d.length > 0);
      // filter the events
      dorks.forEach((dork) => {
        ...
        if (dsl.length == 2) {
          ...
          events = events.filter(
            (e) =>
              e.source.toLowerCase().includes(dork) ||
              e.data.toLowerCase().includes(dork)
          );
        }
      });
        ...
        <div class="tab-pane fade show active" id="events">
            ...
            <div class="flex-fill position-relative">
              <div class="input-group">
                <input
                  ...
                  @input="filterEvents"
                  type="text"
                  class="form-control px-35px rounded"
          ...
          <div class="table-responsive">
            <vue-table
              ...
              :total="eventsTable.totalRecordCount"
